...
# ==========================================
# 4. دوال العرض والتشغيل الحي (Fragments) والشات التفاعلي للبث
# ==========================================
@st.fragment
def render_live_chat(room_id, user_name, is_teacher=False):
    st_autorefresh(interval=2000, key=f"chat_refresh_{room_id}")
    st.markdown("💬 **شات البث المباشر التفاعلي:**")
    
    # زر خاص للأستاذ لمسح شات البث أو إنهاؤه فوراً
    if is_teacher:
        col_end1, col_end2 = st.columns(2)
        if col_end1.button("🗑️ مسح تعليقات البث الآن", key=f"clear_chat_{room_id}"):
            with sqlite3.connect(DB_NAME) as conn:
                c = conn.cursor()
                c.execute("DELETE FROM live_chat WHERE room_id=?", (room_id,))
                conn.commit()
            st.success("تم مسح جميع تعليقات البث وتصفير الشات بنجاح!")
            st.rerun()

    with st.form(f"chat_form_{room_id}", clear_on_submit=True):
        msg = st.text_input("اكتب تعليقك في البث...")
        send_btn = st.form_submit_button("إرسال تعليق")
        if send_btn and msg:
    ...
    try:
        ...
        if messages:
            ...
            st.info("لا توجد تعليقات حالياً في البث. (سيتم حذف التعليقات تلقائياً عند انتهاء البث)")
    except:
        pass
...
def render_student_teacher_card(t_name, t_sub, t_price, room_id, t_phone, student_phone):
    ...
    if sub_status == 'active' and not is_expired:
        ...
        with tab_live:
            ...
            render_live_chat(room_id, st_current_name, is_teacher=False)
...
if not st.session_state.is_logged_in:
    ...
    if st.session_state.user_role == "طالب":
        st.sidebar.markdown(f"📱 هاتفك: `{st.session_state.user_phone}`")
        if st.sidebar.button("🚪 تسجيل الخروج"):
            logout_user()
            st.rerun()

        st.subheader("👨‍🏫 قائمة الأساتذة المتاحين في المنصة")
        try:
            with sqlite3.connect(DB_NAME) as conn:
                c = conn.cursor()
            ...
            if teachers:
                for t_name, t_sub, t_price, room_id, t_phone in teachers:
        ...
        st.sidebar.markdown(f"👨‍🏫 مرحباً بك يا أستاذ")
        if st.sidebar.button("🚪 تسجيل الخروج"):
            logout_user()
            st.rerun()
        ...
        tab_broadcast, tab_subs, tab_upload = st.tabs(["🔴 البث المباشر والشات", "👥 طلبات اشتراك الطلاب", "📤 رفع المحتوى"])
        ...
        with tab_broadcast:
            ...
            # الشات الخاص بالأستاذ مع زر مسح التعليقات فور انتهاء البث
            render_live_chat(room_id, t_name, is_teacher=True)

        with tab_subs:
            st.markdown("### طلبات انضمام الطلاب واشتراكاتهم")
            display_teacher_requests(t_phone)

        with tab_upload:
            st.markdown("### رفع حصة أو فيديو جديد")
            with st.form("upload_form", clear_on_submit=True):
                ...
                up_btn = st.form_submit_button("رفع ونشر للمشتركين")
                    ...
                    if p_title and uploaded_file:
                        ...
                        try:
                            with sqlite3.connect(DB_NAME) as conn:
                                ...
                                conn.commit()
                            st.success("تم رفع ونشر الدرس بنجاح!")
                            st.rerun()
                        except Exception as e:
                            st.error(f"خطأ أثناء الحفظ: {e}")
                    else:
                        st.error("أدخل عنوان الدرس وارفع الملف!")

    elif st.session_state.user_role == "مطور":
        st.sidebar.markdown("👑 **لوحة المطور الرئيسي**")
        if st.sidebar.button("🚪 تسجيل الخروج"):
            logout_user()
            st.rerun()

        st.subheader("إدارة المنصة بالكامل")
        # لوحة المطور الأساسية
        try:
            with sqlite3.connect(DB_NAME) as conn:
                c = conn.cursor()
            ...
            col1.metric("إجمالي الطلاب", users_count)
            col2.metric("إجمالي الأساتذة", teachers_count)
        except:
            pass
